...
export function AdminConsoleView() {
    ...
    const activeTab = useMemo<TabKey>(() => {
        ...
        if (pathname.endsWith('/markets')) return 'markets';
        if (pathname.endsWith('/accounts')) return 'accounts';
        if (pathname.endsWith('/projects')) return 'projects';
        if (pathname.endsWith('/teams')) return 'teams';
        if (pathname.endsWith('/members')) return 'members';
        if (pathname.endsWith('/users')) return 'users';
        if (pathname.endsWith('/onboard-employee')) return 'onboard-employee';
        if (pathname.endsWith('/report-schedules')) return 'report-schedules';
        return filteredTabs[0]?.key || 'teams'; // Fallback
    ...
    // Handle deep-linked action query param for team creation helper
    useEffect(() => {
        const createTeamId = searchParams.get('createTeamId');
        if (createTeamId) {
            handleCreate({ teamId: createTeamId });
            // Clean up URL query params
            const url = new URL(window.location.href);
            url.searchParams.delete('createTeamId');
                ...
                {filteredTabs.map(tab => {
                    ...
                    return (
                        <button
                            ...
                            className={cn(
                                'flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 flex-1 min-w-[140px] justify-center cursor-pointer',
                                isActive
                                                    ...
                                                    <div className="flex gap-1.5 justify-end opacity-0 group-hover:opacity-100 transition-all">
                                                        ...
                                                        {(activeTab === 'teams') && (
                                                            <Button variant="ghost" size="icon" className="h-7 w-7 rounded-lg bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500/20"
                                                                onClick={() => {
                                                                    router.push(`/admin/members?createTeamId=${item.id}`);
                                                                }}>
